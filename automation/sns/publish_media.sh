...
MEDIA_NAME="${MEDIA_NAME:-latest.mp4}"
...
git add "$MEDIA_NAME"
git commit -m "Update BASE SNS preview media"
git remote add origin "https://x-access-token:${GITHUB_TOKEN}@github.com/${GITHUB_REPOSITORY}.git"
git push --force origin "$MEDIA_BRANCH"
popd >/dev/null

MEDIA_URL="https://raw.githubusercontent.com/${GITHUB_REPOSITORY}/${MEDIA_BRANCH}/${MEDIA_NAME}"
printf '%s\n' "$MEDIA_URL" > automation/sns/output/public_media_url.txt
echo "Published media URL prepared."
